...
def load_parent_model_metrics(parent_model, base_dir='eval_res'):
    """Load parent model evaluation metrics (e.g., CARTE teacher performance)."""
...
def create_comparison_plots(baseline_df, distillation_df):
    ...
    for parent_model in parent_models:
        ...
        if parent_model.lower() == 'carte':
            ...
            create_carte_plots(merged_df, parent_model, output_dir, parent_metrics_df)
        else:
...
def create_carte_plots(merged_df, parent_model, output_dir, parent_metrics_df):
    """Create CARTE plot showing baseline, distillation, and teacher performance."""
    ...
    # Overall shot setting comparison including parent model
    shot_comparison = merged_df.groupby(['numshot', 'method'])['mean_auc'].mean().reset_index()
    shot_comparison.columns = ['numshot', 'method', 'mean_auc']
    ...
    if distill_x:
        ...
        plt.plot(distill_x, distill_y,
                 color='red', linestyle='-', marker='s', alpha=0.8,
                 label='Distillation', linewidth=3, markersize=8)
    ...
    if not parent_metrics_df.empty:
        ...
        if parent_x:
            ...
            plt.plot(parent_x, parent_y,
                     color='green', linestyle='-.', marker='^', alpha=0.85,
                     label='CARTE', linewidth=3, markersize=8)

    plt.title(f'{display_name} Performance vs. Shot Size', fontsize=20, fontweight='bold')
    plt.gca().set_xticks(shot_values, minor=True)
    plt.gca().set_xticks(xtick_values, minor=False)
    plt.gca().tick_params(axis='x', which='minor', length=8, width=2)
    plt.gca().tick_params(axis='x', which='major', length=4, width=1)
    ...
    plt.xlim(min(shot_values) * 0.9, max(shot_values) * 1.05)
    plt.xlabel('Number of Shots', fontsize=14)
    plt.ylabel('Mean AUC (averaged across datasets)', fontsize=14)
